...
int main() {
    ...
    deque<ll> pld, mnd;
    for (auto x : pl) pld.pb(x);
    for (auto x : mn) mnd.pb(x);
    ...
    auto f = [&](ll a, ll b, ll c) -> double {
        double su = a + b + c;
        double pr = a * b * c;
        return su / pr;
    };

    double mi = f(U[0], U[1], U[2]), ma = f(U[0], U[1], U[2]);

    for (int i = 0; i < U.size(); i++) {
        for (int j = i + 1; j < U.size(); j++) {
            for (int k = j + 1; k < U.size(); k++) {
                chmin(mi, f(U[i], U[j], U[k]));
                chmax(ma, f(U[i], U[j], U[k]));
            }
        }
    }
