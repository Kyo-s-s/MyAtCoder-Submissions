...
struct S {
    ...
    S(vector<string> v) {
        ...
        W = v[0].size();
        // トリミング
        ll hmax = 0, hmin = 1e9, wmax = 0, wmin = 1e9;
        rep(h, H) rep(w, W) if (v[h][w] == '#') {
            chmax(hmax, h);
            chmin(hmin, h);
            chmax(wmax, w);
            chmin(wmin, w);
        ...
        W = wmax - wmin + 1;
        F.resize(H);
        rep(h, H) rep(w, W) F[h].pb(v[h + hmin][w + wmin]);
    }

    bool operator==(const S &s) const {
        if (H != s.H || W != s.W) return false;
        rep(h, H) rep(w, W) if (F[h][w] != s.F[h][w]) return false;
        return true;
    }
...
int main() {
    ...
    rep(h, x.H - a.H + 1) rep(w, x.W - a.W + 1) {
        // a の置く位置
        vector<string> v(x.H, string(x.W, '.'));
        // a を h, w ずらして置く
        rep(hh, a.H) rep(ww, a.W) v[h + hh][w + ww] = a.F[hh][ww];
        rep(s, x.H - b.H + 1) rep(t, x.W - b.W + 1) {
            auto u = v;
            rep(hh, b.H) rep(ww, b.W) if (b.F[hh][ww] == '#') u[s + hh][t + ww] = b.F[hh][ww];
            S tmp = S(u);
            if (tmp == x) {
                OUT("Yes");
                return 0;
