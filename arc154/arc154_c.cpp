...
void solve() {
    ...
    bool notloop = true;
    vll cntA(N + 1, 0), cntB(N + 1, 0);
    for (auto a : A) cntA[a]++;
    for (auto b : B) cntB[b]++;

    rep(i, N + 1) if (cntA[i] > 0 && cntB[i] == 0) notloop = false;
    rep(i, N) if (A[i] == A[(i + 1) % N]) notloop = false;

    vvll nxtidxA(N + 1, vll(N + 1, INF));
    repd(i, N) {
        rep(j, N + 1) nxtidxA[i][j] = nxtidxA[i + 1][j];
        nxtidxA[i][A[i]] = i;
    } 
    ...
    rep(st, N + 10) {
        ...
        rep(i, N) {
            ll val = B[(st + i) % N];
            ll idx_nxt = nxtidxA[max(0LL, idx)][val];
            if (idx_nxt > idx + 1) ch = true;
            idx = idx_nxt;
            if (idx == INF) break;
        }
