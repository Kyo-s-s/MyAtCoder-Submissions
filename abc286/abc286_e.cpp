...
int main() {
    ...
    // 経路、価値総和
    vector ans(N, vector<pair<ll, ll>>(N, {INF, -INF}));
    ...
    rep(i, N) {
        ...
        while (!que.empty()) {
            ...
            rep(j, N) {
                if (S[now][j] == 'Y') {
                    if (ans[i][j].fi > ans[i][now].fi + 1) {
                        ans[i][j].fi = ans[i][now].fi + 1;
                        ans[i][j].se = ans[i][now].se + A[j];
                        que.pb(j);
                    } else if (ans[i][j].fi == ans[i][now].fi + 1) {
                        chmax(ans[i][j].se, ans[i][now].se + A[j]);
                    }
                }
            }
        }
