...
template<class T = int> struct Tree {
    ...
    vector<T> calc_dist(int s = 0) {
        vector<T> dist(G.size(), -1); dist[s] = 0;
...
int main () {
    ...
    LL(Q);
    vector Que(N, vector<pll>(0));  // Que[i] := i に対するクエリ　{k, idx}
    rep (i, Q) {
        LL(u, k);
        u--;
        Que[u].pb({k, i});
    ...
    auto [_, L, R] = tree.diameter();
    

    auto dfs = [&N, &G, &Que, &Ans](auto&& self, ll now, ll bef, vll &stack) -> void {
        for (auto [k, idx]: Que[now]) {
            if (int(stack.size()) >= k) {
                Ans[idx] = stack[stack.size() - k];
            }
        }

        stack.pb(now);
        for (auto e: G[now]) if (bef != e) {
            self(self, e, now, stack);
        }
        stack.pop_back();
    };
