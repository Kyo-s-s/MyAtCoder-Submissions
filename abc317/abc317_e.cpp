...
int main() {
    ...
    auto go = [&](int h, int w) {
        auto [dh, dw] = dis(A[h][w]).value();
        while (true) {
            ll nh = h + dh, nw = w + dw;
            if (!(include(0, H, nh) && include(0, W, nw))) break;
            if (!(A[nh][nw] == '.' || A[nh][nw] == 'x')) break;
            h = nh, w = nw;
            A[h][w] = 'x';
        }
    };
    ...
    dist[s.fi][s.se] = 0;
    vpii dhw = { {0, 1}, {1, 0}, {0, -1}, {-1, 0} };
    while (!que.empty()) {
        auto [h, w] = que.front(); que.pop_front();
        for (auto [dh, dw] : dhw) {
            int nh = h + dh, nw = w + dw;
            if (!(include(0, H, nh) && include(0, W, nw))) continue;
            if (A[nh][nw] == '.' || A[nh][nw] == 'G') {
                if (chmin(dist[nh][nw], dist[h][w] + 1)) {
                    que.push_back(make_pair(nh, nw));
                }
            }
        }
    }

    if (dist[g.fi][g.se] == INF) {
        OUT(-1);
    } else {
        OUT(dist[g.fi][g.se]);
    }
