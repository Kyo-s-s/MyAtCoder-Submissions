...
void solve() {
    ...
    CumulativeSum<ll> U(N), T(N), P(N), C(N);
    rep(i, N) {
        if (S[i] == 'U') U.add(i, 1);
        if (S[i] == 'T') T.add(i, 1);
        if (S[i] == 'P') P.add(i, 1);
    ...
    optional<int> ut = nullopt;
    optional<int> t = nullopt;
    ll cu = 0, cc = 0, ct = 0, cp = 0;
    ll u = 0, c = 0, p = 0;
    ...
    for (int i = 1; i < N - 1; i++) {
        ...
        [&]{
            if (t && S[i] == 'P') {
                ll cost = 0;
                if (C.sum(i, N) == 0) return;
                if (U.sum(0, t.value()) == 0) return;
                if (u > 0) {
                    cost += 2; 
                } else {
                    if (S[i + 1] != 'C') cost++;
