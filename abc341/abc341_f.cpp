...
int main() {
    ...
    auto solve = [&](auto&& self, ll x) {
        if (memo[x] != -1) return memo[x];
        // ナップサックを解く
        vll dp(5050, 0);
        // dp[w] := w の重さ以下の操作回数
        for (auto e : G[x]) if (W[e] < W[x]) {
            // 高々 5000 回しか呼ばれない
            ll w = self(self, e);
            for (ll i = 5049; i >= 0; i--) {
                if (i + W[e] <= 5050) {
                    chmax(dp[i + W[e]], dp[i] + w);
                }
                // if (i - 1 >= 0) chmax(pd[i], dp[i - 1]);
            }
        }
        ll u = 0;
        rep(i, W[x]) chmax(u, dp[i]);
        memo[x] = u + 1;
        return memo[x];
    };
    ...
    ll ans = 0;
    rep(i, N) {
        ans += A[i] * solve(solve, i);
        // OUT("deb: ", i, solve(solve, i));
    }
    OUT(ans);
