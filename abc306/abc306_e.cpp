...
struct Max_M {
    ...
    static T op(T l, T r) { return l.max > r.max ? l : r; }
    static T e() { return T{-INF, -1}; }
    static T gen(ll idx) { return T{-INF, idx}; }
    static T gen(ll val, ll idx) { return T{val, idx}; }
};
...
struct Min_Sum_M {
    ...
    static T op(T l, T r) {
        return T{
            min(l.min, r.min),
            l.sum + r.sum,
            l.min < r.min ? l.idx : r.idx
        };
    }
    static T e() { return T{INF, 0, -1}; }
    static T gen(ll idx) { return T{INF, 0, idx}; }
    static T gen(ll val, ll idx) { return T{val, val, idx}; }
};
...
int main() {
    ...
    // used: true
    vector<Max_M::T> vma(N);
    // true
    vector<Min_Sum_M::T> vmi(N);
    rep(i, N) {
        if (used[i]) {
            vma[i] = Max_M::gen(i);
            vmi[i] = Min_Sum_M::gen(A[i], i);
        } else {
            vma[i] = Max_M::gen(A[i], i);
            vmi[i] = Min_Sum_M::gen(i);
        }
    }
    Segtree<Max_M> segma(vma);
    Segtree<Min_Sum_M> segmi(vmi);

    while (Q--) {
        LL(x, y); x--;
        A[x] = y;
        if (used[x]) {
            segmi.set(x, Min_Sum_M::gen(y, x));
        } else {
            segma.set(x, Max_M::gen(y, x));
        }

        auto ma = segma.all_prod();
        auto mi = segmi.all_prod();
        if (ma.max > mi.min) {
            used[mi.idx] = false;
            segmi.set(mi.idx, Min_Sum_M::gen(mi.idx));
            segma.set(mi.idx, Max_M::gen(mi.min, mi.idx));

            used[ma.idx] = true;
            segmi.set(ma.idx, Min_Sum_M::gen(ma.max, ma.idx));
            segma.set(ma.idx, Max_M::gen(ma.idx));
        }

        OUT(segmi.all_prod().sum);
