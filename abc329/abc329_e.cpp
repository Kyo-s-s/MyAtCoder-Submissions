...
int main() {
    ...
    for (auto s : S) {
        L.back().pb(s);
        // 末尾が T なら消して空文字を追加
        if (L.back().size() >= M && L.back().substr(L.back().size() - M) == T) {
            for (int i = 0; i < M; i++) L.back().pop_back();
            L.pb("");
        }
    }
    ...
    for (auto l : L) {
        ...
        l = "#####" + l;
        while (l.size() > 0) {
            dq.push_front(l.back());
            l.pop_back();
            if (dq.size() <= M) {
                bool ok = true;
                rep(i, M) {
                    if (!(i >= dq.size() || dq[i] == T[i] || dq[i] == '#')) {
                        ok = false;
                        break;
                    }
                }
