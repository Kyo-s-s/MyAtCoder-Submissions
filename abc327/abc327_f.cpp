...
int main() {
    ...
    sort(all(pos), [&](pll a, pll b) {
        if (a.se == b.se) return a.fi < b.fi;     
        return a.se < b.se;
        // if (a.fi == b.fi) return a.se < b.se;
        // return a.fi < b.fi;
    });
    LazySegtree<M, O, fn> seg(vll(402020, 0));
    deque<pll> yet, now;
    fore(p, pos) yet.push_back(p);

    auto get_range = [&](ll t, ll x) -> pll {
        ll l = t;
        ll r = min(402020LL, t + D);
        return pll{l, r};
    };



    auto ins = [&](ll t, ll x) -> void {

        auto [l, r] = get_range(t, x);
        seg.apply(l, r, 1);
    };

    auto dec = [&](ll t, ll x) -> void {
        auto [l, r] = get_range(t, x);
        seg.apply(l, r, -1);
    };
    ...
    while (!yet.empty()) {
        auto [t, x] = yet.front(); yet.pop_front();
        while (!now.empty()) {
            auto [s, w] = now.front();
            if (x - W + 1 <= w) {
                break;
