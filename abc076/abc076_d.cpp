...
int main() {
    ...
    vll Ts = {0};
    for (auto t : T) Ts.pb(Ts.back() + t);
    set<ll> sT(all(Ts));


    // dp[v] := 今速度がvであるような、移動距離の最大 存在しないなら -INF
    // 100まで持てばよい。

    vector<double> dp(210, -1e9);
    dp[0] = 0;

    int idx = -1;
    for (int t = 0; t < Ts.back(); t++) {
        // イベント？
        if (sT.count(t)) idx++;
        // lim -> V[idx] まで

        vector<double> pd(210, -1e9);
        for (int i = 0; i <= V[idx]; i++) {
            chmax(pd[i], dp[i] + i * 0.25);
            if (i + 1 <= V[idx]) {
                chmax(pd[i + 1], dp[i] + i * 0.25 + 0.25);
            }
            if (0 <= i - 1) {
                chmax(pd[i - 1], dp[i] + i * 0.25 - 0.25);
            }
