...
ll solve(ll A, ll B) {
    ...
    while (true) {
        ...
        ll a = A / g;
        ll b = B / g;
        vll div = divisor(max(a - b, b - a));
        
        ll k = 1;
        // a % k == b % k となり、最小の a % k が採用される
        ll mi = INF;
        ll val = 0;
        for (ll d : div) if (d != 1) {
            if (a % d == b % d) {
...
int main() {
    ...
    // rep(i, 10) {
    //     // 10^10以上 10^12以下の乱数生成
    //     ll A = rand() % 1000000000 + 10000000000;
    //     ll B = rand() % 1000000000 + 10000000000;
    //     ll ans = solve(A, B);
    // }
