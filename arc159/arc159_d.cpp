...
int main() {
    ...
    rep(i, N) {
        ...
        // se を縮める？
        if (prev_r > l) {
            del.push_back({prev_l, prev_r});
            add.push_back({prev_l, l});
            del_num -= (prev_r - l);
        }
        ...
        while (del_num > 0) {
            iter++;
            ll c = iter->se - iter->fi;
            if (c <= del_num) {
                del.push_back(*iter);
                del_num -= c;
            } else {
                del.push_back(*iter);
                add.push_back({iter->fi + del_num, iter->se});
                del_num = 0;
            }
        }
        for (auto d : del) {
