...
int main() {
    ...
    LL(N);   
    min_priority_queue<pll> task;
    min_priority_queue<ll> can;
    rep(i, N) {
        LL(t, d);
        task.push({t, t + d});
    }
    ...
    while (!task.empty()) {
        ll t = task.top().fi;
        while (!task.empty() && task.top().fi == t) {
            can.push(task.top().se);
            task.pop();
        }
        
        ll end = (task.empty() ? INF : task.top().fi);
        while (!can.empty() && t < end) {
            ll d = can.top(); can.pop();
            if (t <= d) {
                ans++;
                t++;
            }
