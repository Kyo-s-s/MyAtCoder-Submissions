...
int main() {
    ...
    ll mod = 998244353;
    vector<set<ll>> G(N + 10);

    auto add = [](ll a, ll b) { return a + b; };
    auto rev = [](ll a) { return a; };

    auto lct = get_link_cut_tree<ll>(
        add, rev
    );
    ...
    auto solve = [&](ll t, ll u, ll v) -> optional<ll> {
        // OUT("Q", t, u, v);
        if (t == 1) {
            G[u].insert(v);
            G[v].insert(u);
            if (!lct.is_connected(vs[u], vs[v])) {
                lct.evert(vs[u]);
                lct.link(vs[u], vs[v]);
            }
            return nullopt;
            ...
            lct.evert(vs[u]);
            auto p = lct.get_kth(vs[v], 1);
            auto pp = lct.get_kth(vs[v], 2);
            if (pp != nullptr && pp->key == u) {
                return p->key;
            }
            return 0;
        }
    ...
    while (Q--) {
        LL(a, b, c);
        ll A = 1 + (((a * ((1 + X.back()) % mod)) % mod) % 2);
        ll B = 1 + (((b * ((1 + X.back()) % mod)) % mod) % N);
        ll C = 1 + (((c * ((1 + X.back()) % mod)) % mod) % N);
