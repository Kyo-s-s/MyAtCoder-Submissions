...
template<class T> struct Vec : public std::vector<T> {
    ...
    bool all_of(std::function<bool(T)> f) {
        return std::all_of(this->begin(), this->end(), f);
    }

    bool any_of(std::function<bool(T)> f) {
    ...
    inline const T &operator[](int k) const { return (this->at(k)); }
    inline T &operator[](int k) { return (this->at(k)); }

    friend std::ostream &operator<<(std::ostream &os, Vec<T> &p) {
