...
int main() {
    ...
    vvll dist(sz, vll(sz, INF));
    
    auto update = [&](int idx) {
        deque<pll> que;
        que.pb(O[idx]);
        vvll di(H, vll(W, INF));
        di[O[idx].fi][O[idx].se] = 0;
        ...
        while (!que.empty()) {
            ...
            for (auto [dx, dy] : dxy) {
                ...
                if (A[nx][ny] == '#') continue;
                if (di[nx][ny] <= di[h][w] + 1) continue;
                di[nx][ny] = di[h][w] + 1;
                que.pb({nx, ny});
            }
        }
        rep(i, sz) {
            dist[idx][i] = di[O[i].fi][O[i].se];
        }
    };
    ...
    for (ll msk = 0; msk < (1 << n); msk++) {
        for (ll v = 0; v < n; v++) {
            for (ll u = 0; u < n; u++) {
                if (msk & (1 << u)) {
                    dp[msk][v] = min(dp[msk][v], dp[msk ^ (1 << v)][u] + d(u, v));
                }
            }
        }
    }
    ...
    for (ll msk = 0; msk < (1 << n); msk++) {
        for (ll v = 0; v < n; v++) {
            if (dp[msk][v] + dist[v][n] <= T) {
                ll tmp = __builtin_popcount(msk) - 1;
                ans = max(ans, tmp);
            }
        }
    }
