...
struct Node {
    char op;
    Mint val;
    optional<Node_ptr> left, right;
    Node(
        char op, Mint val,
        optional<Node_ptr> left,
        optional<Node_ptr> right
    ) : op(op), val(val) {
        this->left = move(left);
...
struct Parser {
    ...
    Parser(const string& s, const vector<Mint>& a) : S(s), A(a), idx(0), aidx(0) {
        root = expr();
        // debug(root);
        // cout << endl;
        dp.resize(A.size());
    ...
    void debug(Node_ptr& node) {
        if (node->op == '!') {
            cout << "a";
            return;
        }
        Node_ptr& left = node->left.value();
        Node_ptr& right = node->right.value();
        cout << "(";
        debug(left);
        cout << node->op;
        debug(right);
    ...
    Mint query(int i, Mint x) {
        return root->val + (dp[i] * (x - A[i]));
    }

    void dfs(Node_ptr& node, Mint w) {
        if (node->op == '!') {
            dp[aidx++] = w;
            return;
        }  
        ...
        if (node->op == '+') {
            ...
            dfs(left, w * right->val);
            dfs(right, w * left->val);
        }
    }
    ...
    Node_ptr expr() {
        ...
        while (true) {
            if (consume('+')) {
                Node_ptr right = term();
                Mint val = res->val + right->val;
                res = Node_ptr(new Node('+', val, move(res), move(right)));
            } else if (consume('-')) {
                Node_ptr right = term();
                Mint val = res->val - right->val;
                res = Node_ptr(new Node('-', val, move(res), move(right)));
    ...
    Node_ptr value() {
        ...
        if (consume('a')) {
            return Node_ptr(new Node('!', A[aidx++], nullopt, nullopt));
        }
        assert(false);
    }
...
int main() {
    ...
    while (Q--) {
        INT(i);
        LL(x);
        cout << parser.query(i - 1, Mint(x)) << endl;
    }    
