...
int main() {
    ...
    auto check = [&](char c, int type) {
        if (type == 1) {
            rep(i, 4) {
                if (c == one[i]) return true;
            }
        } else {
            rep(i, 13) {
                if (c == two[i]) return true;
            }
        }
        return false;
    };

    for (auto s: S) if (check(s[0], 1) == false) f1 = false;
    for (auto s: S) if (check(s[1], 2) == false) f2 = false;

    set<string> st;
    for (auto s: S) st.insert(s);
    if (st.size() != N) f3 = false;
