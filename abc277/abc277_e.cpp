...
int main() {
    ...
    vvll dist(N, vll(2, INF)); // dist[i][j] := i に 今 j の反転状況で到達する最短距離

    queue<pll> que;
    que.push({0, 0}); // 0: Gt, 1: Gf
    dist[0][0] = 0;
    while (!que.empty()) {
        auto [n, flg] = que.front(); que.pop();

        if (flg == 0) {
            // そのまま
            for (auto e: Gt[n]) {
                if (dist[e][flg] > dist[n][flg] + 1) {
                    dist[e][flg] = dist[n][flg] + 1;
                    que.push({e, flg});
                }
            }
            // 反転できる？
            if (sw.find(n) != sw.end()) {
                for (auto e: Gf[n]) {
                    if (dist[e][1] > dist[n][flg] + 1) {
                        dist[e][1] = dist[n][flg] + 1;
                        que.push({e, 1});
                    }
                }
            }
            ...
            if (sw.find(n) != sw.end()) {
                for (auto e: Gt[n]) {
                    if (dist[e][0] > dist[n][flg] + 1) {
                        dist[e][0] = dist[n][flg] + 1;
                        que.push({e, 0});
                    }
                }
            }
    ...
    ll ans = INF;
    chmin(ans, dist[N - 1][0]);
    chmin(ans, dist[N - 1][1]);
    if (ans == INF) ans = -1;
