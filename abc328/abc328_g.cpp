...
int main() {
    ...
    vll values;
    rep(i, (1 << N)) values.pb(i);
    sort(all(values), [](ll a, ll b) { return __builtin_popcountll(a) < __builtin_popcountll(b); });
    ...
    for (auto msk : values) {
        ...
        for (ll w = 1; w + i <= N; w++) {
            hav.pb(A[i + w - 1]);
            // 今幅 w で持ってる msk について、連続 w 個0ならそこに入れて遷移
            ll wmsk = (1 << w) - 1;
            // cout << s << " " << bitset<5>(wmsk) << endl;
            for (ll j = 0; j + w <= N; j++) {
                if ((msk | (wmsk << j)) == (msk + (wmsk << j))) {
                    ll nmsk = msk | (wmsk << j);
                    ll ncost = cost;
                    for (int k = 0; k < w; k++) {
                        ncost += abs(hav[k] - B[j + k]);
                    }
                    ncost -= (w - 1) * C;
                    chmin(dp[nmsk], ncost);
                }
