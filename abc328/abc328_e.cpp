...
int main() {
    ...
    while (!dq.empty()) {
        auto [S, cost] = dq.front(); dq.pop_front();
        if (S == (1 << N) - 1) {
            chmin(ans, cost);
            continue;
        }
        rep(i, N) if ((S >> i) & 1) {
            for (auto [e, w] : G[i]) if (!((S >> e) & 1)) {
                ll T = S | (1 << e);
                ll new_cost = (cost + w) % K;
                if (st.count({T, new_cost})) continue;
                st.insert({T, new_cost});
                dq.push_back({T, new_cost});
            }
        }
    }
