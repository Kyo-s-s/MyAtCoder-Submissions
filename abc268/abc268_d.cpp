...
int main() {
    ...
    ll strlensum = 0;
    for (auto s: S) strlensum += s.length();
    ll canuseunderbar = 16 - strlensum - (N - 1);

    string ans = "-1";

    vvll barnum;

    auto bar = [&](auto&& self, vll add, ll su) -> void {
        if (add.size() == N-1) {
            barnum.pb(add);
            return;
        }
        for (int use = 0; use <= canuseunderbar; use++) {
            add.pb(use);
            self(self, add, su + use);
            add.pop_back();
        }
    };
    ...
    do {
        ...
        for (auto b: barnum) {
            ...
            if (!(3 <= tmp.length() && tmp.length() <= 16)) continue;
