...
int main() {
    ...
    rep(idx, (int)T.size()) {
        char t = T[idx];
        if (t == '+') {
            for (int cp = 0; cp <= min(N - P, q); cp++) {
                ll le = notqsum + cp - (q - cp);
                ll ri = - le - 1;
                // 右
                if (le - ri > 0) {
                    ans += -idx * enu.nCk(q, cp) * enu.nCk(Q - q, N - P - cp);
                } else if (le - ri < 0) {
                    ans += idx * enu.nCk(q, cp) * enu.nCk(Q - q, N - P - cp);
                }
            }
        ...
        if (t == '?') {
            // + と決めたとき
            for (int cp = 0; cp <= min(N - P - 1, q); cp++) {
                ll le = notqsum + cp - (q - cp);
                ll ri = - le - 1;
                if (le - ri > 0) {
                    ans += -idx * enu.nCk(q, cp) * enu.nCk(Q - q - 1, N - P - cp - 1);
                } else if (le - ri < 0) {
                    ans += idx * enu.nCk(q, cp) * enu.nCk(Q - q - 1, N - P - cp - 1);
                }
            }
            for (int cp = 0; cp <= min(N - P, q); cp++) {
                ...
                ll ri = - le + 1;
                if (le - ri < 0) {
                    ans += -idx * enu.nCk(q, cp) * enu.nCk(Q - q - 1, N - P - cp);
                } else if (le - ri > 0) {
                    ans += idx * enu.nCk(q, cp) * enu.nCk(Q - q - 1, N - P - cp);
                }   
            }
