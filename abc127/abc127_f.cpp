...
struct Median {
    priority_queue<ll> sm;
    min_priority_queue<ll> lg;

    // sm.top() と sm の中の差の総和　とそれぞれ持っておけば楽？
    ll diff_sm, diff_lg;


    Median() : diff_sm(0), diff_lg(0) {
    }
    ...
    ll pop_sm() {
        ll ret = sm.top();
        sm.pop();
        if (!sm.empty() && sm.top() < ret) {
            diff_sm -= (ret - sm.top()) * sm.size();
        }
        return ret;
    }
    ...
    ll pop_lg() {
        ll ret = lg.top();
        lg.pop();
        if (!lg.empty() && ret < lg.top()) {
            diff_lg -= (lg.top() - ret) * lg.size();
        }
        return ret;     
    }

    ll val() {
        return diff_sm + (lg.top() - median()) * lg.size() + diff_lg;
    }
    ...
    void push(ll x) {
        ...
        while (
            !(
                (sm.size() == lg.size() || sm.size() == lg.size() + 1) &&
                (lg.empty() || (!sm.empty() && sm.top() <= lg.top()))
            )
        ) {
            if (!sm.empty() && !lg.empty() && sm.top() > lg.top())  {
                push_lg(pop_sm());
            }
            if (sm.size() < lg.size()) {
                push_sm(pop_lg());
...
int main() {
    ...
    while (Q--) {
        ...
        if (t == 1) {
            ...
            OUT(med.median(), med.val() + B);
        }
        // OUT("debug", med.median(), med.val(), B);
    }
}
