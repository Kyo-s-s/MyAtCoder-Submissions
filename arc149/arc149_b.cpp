...
int main() {
    ...
    auto solve = [](const vector<T> &arr) -> ll{
        vector<ll> lisa, lisb;
        for (auto [a, b]: arr) {

            auto ita = lower_bound(all(lisa), a);
            if (end(lisa) == ita) lisa.pb(a);
            else *ita = a;

            auto itb = lower_bound(all(lisb), b);
            if (end(lisb) == itb) lisb.pb(b);
            else *itb = b;
        ...
        return lisa.size() + lisb.size();

    };

    ll ans = 0;

    sort(all(AB), [](const T &a, const T &b) { return a.a < b.a; });
    chmax(ans, solve(AB));

    sort(all(AB), [](const T &a, const T &b) { return a.b < b.b; });
    chmax(ans, solve(AB));
