...
int main() {
    ...
    vector memo(N, optional<pair<ll, bool>>(nullopt));

    auto dp = [&](auto&& dp, ll n) -> pair<ll, bool> {
        if (memo[n]) return memo[n].value();
        ll res = 1;
        // 先にtrueのやつについてやる、falseのやつは交互になる
        min_priority_queue<ll> pq;
        ll minus = 0, plus = 0;
        for (auto c : Child[n]) {
            auto [r, f] = dp(dp, c);
            if (f) {
        ...
        memo[n] = make_pair(res, k != 1);
        // cout << n << " " << res << " " << k << endl;
        return memo[n].value();
    };

    auto [res, flag] = dp(dp, 0);
    OUT((N + res) / 2);
