...
int main() {
    ...
    auto route = [&](ll x, ll y, ll nx, ll ny) -> Mint {
        ll dx = abs(nx - x), dy = abs(ny - y);
        return enu.nCk(dx + dy, dx);
    };    

    Mint ans = route(0, 0, H - 1, W - 1);

    for (int w = 0; w < B; w++) {
        ans -= route(0, 0, H - A - 1, w) * route(H - A, w, H - 1, W - 1);
    }
