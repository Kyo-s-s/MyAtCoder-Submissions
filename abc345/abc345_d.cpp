...
void solve(
    ll H, ll W,
    vector<pll> pieces // 必ず総和が H * W になる
) {
    ...
    auto dfs = [&](auto&& self, int i) {
        ...
        auto put = [&](ll a, ll b) {
            for (ll h = 0; h < H - a + 1; h++) {
                for (ll w = 0; w < W - b + 1; w++) {
                    if ([&]{
                        rep(i, a) rep(j, b) if (board[h + i][w + j]) return true;
                        return false;
                    }()) {
                        continue;
                    }
                    ...
                    cnt += a * b;
                    self(self, i + 1);
                    rep(i, a) rep(j, b) board[h + i][w + j] = false;
                    cnt -= a * b;
                }
            }
        };

        put(pieces[i].fi, pieces[i].se);
        if (pieces[i].fi != pieces[i].se) put(pieces[i].se, pieces[i].fi);
    };
...
int main() {
    ...
    rep(msk, (1 << N)) {
        ll su = 0;
        rep(j, N) if ((msk >> j) & 1) su += AB[j].fi * AB[j].se;
        if (su != H * W) continue;
        vector<pll> pieces;
        map<pll, ll> a;
        rep(j, N) if ((msk >> j) & 1) {
