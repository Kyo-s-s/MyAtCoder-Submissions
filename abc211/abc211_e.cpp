...
using bit = bitset<64>;
struct bitsetcomp {
    bool operator()(const bit &a, const bit &b) const {
...
int main() {
    ...
    bit wall = 0;
    rep(i, N) rep(j, N) if (S[i][j] == '#') wall.set(8 * i + j, true);

    auto wallcheck = [&](bit s) -> bool {
        return (s & wall) == 0;
    };

    vpll dxy = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};

    auto addcheck = [&](bit s, int x, int y) -> bool {
        if (s.test(8 * x + y)) return false;
        if ([&]() {
            for (auto [dx, dy] : dxy) {
                int nx = x + dx;
                int ny = y + dy;
                if (include(0, N, nx) && include(0, N, ny)) {
                    if (s.test(8 * nx + ny)) return true;
                }
            }
            return false;
        }()) {
    ...
    set<bit, bitsetcomp> st;
    // unordered_set<bit> st;
    rep(i, N) rep(j, N) {
        bit a = 0;
        a.set(8 * i + j, true);
        if (wallcheck(a)) st.insert(a);
    }
    

    rep(k, K - 1) {
        set<bit, bitsetcomp> nst;
        // unordered_set<bit> nst;
        for (auto s : st) {
            rep(i, N) rep(j, N) {
                if (addcheck(s, i, j)) {
                    bit ns = s;
