...
int main() {
    ...
    CumulativeSum<ll> cA(A), cB(B), cC(C), cD(D);
    ...
    auto solve = [&]() {
        ...
        ll ar = [&](){
            // cCについて、opc を超えない最大のもの
            ll ok = 0, ng = A.size() + 1;
            while (abs(ok - ng) > 1) {
                ll mid = (ok + ng) / 2;
                if (cC.sum(mid) <= opc) ok = mid;
                else ng = mid;
            }
            return min(ok, M - use);
        }();
        ...
        ll br = [&](){
            ll ok = ar, ng = A.size() + 1;
            while (abs(ok - ng) > 1) {
                ll mid = (ok + ng) / 2;
                if (cD.sum(ar, mid) + ar + use <= M) ok = mid;
                else ng = mid;
            }
            return ok;
        }();

        ll an = cA.sum(0, ar) + cB.sum(ar, br);
        chmax(ans, an); 
