...
template<class T> struct SortedVec : public std::vector<T> {
    using std::vector<T>::vector;

    SortedVec(vector<T> v) {
    ...
    // x 以上の要素の個数
    int geq_count(T x) {
        return this->end() - std::lower_bound(this->begin(), this->end(), x);
    }
...
int main() {
    ...
    SortedVec<ll> sA(A), sB(B);


    auto f = [&](ll x) -> bool {
        int a = sA.leq_count(x);
        int b = sB.geq_count(x);
        return a >= b;
    };

    ll ok = 1e10, ng = 0;
    ll ans = kyo::bisect(ok, ng, f);
