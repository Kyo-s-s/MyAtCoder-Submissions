...
int main() {
    ...
    LL(N);        
    vector B(110, vector(110, vector(110, (int)-1)));
    vector<tuple<int, int, int, int, int, int>> v;

    rep(n, N) {
        LL(x1, y1, z1, x2, y2, z2);
        x1++; y1++; z1++; x2++; y2++; z2++;
        v.pb({x1, y1, z1, x2, y2, z2});
        for (int i = x1; i < x2; i++) {
            for (int j = y1; j < y2; j++) {
                for (int k = z1; k < z2; k++) {
                    B[i][j][k] = n;
    ...
    for (auto [x1, y1, z1, x2, y2, z2] : v) {
        U.clear();
        for (int i = x1; i < x2; i++) {
            for (int j = y1; j < y2; j++) {
                for (int k = z1; k < z2; k++) {
                    check(B[i][j][k], B[i + 1][j][k]);
                    check(B[i][j][k], B[i][j + 1][k]);
                    check(B[i][j][k], B[i][j][k + 1]);
                    check(B[i][j][k], B[i - 1][j][k]);
                    check(B[i][j][k], B[i][j - 1][k]);
                    check(B[i][j][k], B[i][j][k - 1]);
                }
