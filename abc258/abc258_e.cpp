...
int main(){
    ...
    // vll v;
    // rep(i, 2) for(auto w: W) v.pb(w);

    // segtree<S, op, e> seg(v);

    vll count(N); // iからスタートでいくつのジャガイモが入るか？
    ll alljag = sum(W);
    ll cnt = N * (X / alljag);
    maxjag  = X % alljag;

    ll r = 0, su = 0;
    vvll dab(60, vll(N)); //次のidx
    // debug(maxjag);
    vll v;
    // rep(x, 3) for(auto w: W) v.pb(w);

    rep(i, N){
        // cerr << i << " " << r << " " << su << endl;
        while(su < maxjag) su += W[r++ % N];
        count[i] = cnt + (r - i);
        dab[0][i] = r % N;
        su -= W[i];
    }
    ...
    // rep(x, 2){
    //     for(int i = 0; i < dab[x].size(); i++){
    //         cout << dab[x][i] << (i != dab[x].size() - 1 ? ' ' : '\n');
    //     }
    // }
