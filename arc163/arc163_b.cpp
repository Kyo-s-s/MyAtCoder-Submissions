...
int main() {
    ...
    for (auto a : As) {
        ...
        ll lookidx = M;
        {
            auto it = lower_bound(all(Ap), a);
            ll c2 = it - Ap.begin();
            lookidx += c2;
        }

        // cout << "lookidx: " << lookidx << endl;
        if (lookidx > (int)Ap.size()) continue;
        // ll c2 = abs(A2 - Ap[lookidx - 1]);
        ll c2 = A2 < Ap[lookidx - 1] ? Ap[lookidx - 1] - A2 : 0;

        // cout << a << " " << c1 << " " << c2 << " " << lookidx << endl;
        chmin(ans, c1 + c2);
    }
