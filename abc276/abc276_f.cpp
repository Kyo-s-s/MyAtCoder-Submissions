...
int main() {
    ...
    segtree<Mint, op, e> segwa(202020);
    segtree<ll, lop, le> segcn(202020);
    ...
    rrep(o, N) {
        ...
        // 自分より大きい要素
        Mint big = segwa.prod(a + 1, 202020);
        sum += (big * 2LL);

        // 自分より小さい要素の数
        ll small = segcn.prod(0, a + 1);
        sum += (small * a * 2LL);

        // OUT(sum * Mint(i * i).inv().val());
        // debug(sum);
        Mint ans = sum * Mint(i * i).inv();
        OUT(ans.val());


        segwa.set(a, segwa.get(a) + a);
        segcn.set(a, segcn.get(a) + 1LL);
