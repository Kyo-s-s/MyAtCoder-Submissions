...
vvll mv(const vvll& data, ll u) {
    ll x = u / 2, y = u % 2;
    vvll res = data;
    rep(i, H - 1) rep(j, W - 1) {
        if (!include(0, H, H-i+x-2) || !include(0, W, W-j+y-2)) {
            OUT("error", i, j, x, y);
            exit(1);
        }
        res[i+x][j+y] = data[H-i+x-2][W-j+y-2];
    }
    return res;
}

map<vvll, ll> generate10(vvll &data) {
    map<vvll, ll> res;
    set<vvll> s = {data};
    res[data] = 0;
    for (int c = 1; c <= 10; c++) {
        set<vvll> nxt;
        for(auto &v : s) rep(u, 4) {
            vvll add = mv(v, u);
            if (res.count(add)) continue;
            res[add] = c;
            nxt.insert(add);
        }
        s = nxt;
...
int main() {
    ...
    vvll T(H, vll(W));
    rep(h, H) rep(w, W) T[h][w] = h * W + w + 1;

    map<vvll, ll> s = generate10(S);
    map<vvll, ll> t = generate10(T);

    ll ans = INF;
    for (auto &p : s) {
        if (t.count(p.fi)) {
            chmin(ans, p.se + t[p.fi]);
        }
    }
