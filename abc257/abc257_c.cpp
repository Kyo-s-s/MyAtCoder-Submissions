...
int main(){
    ...
    vector<pair<ll, bool>> D;
    rep(i, N) D.pb({W[i], S[i] == '1'});
    ...
    vll sortW;
    rep(i, N) sortW.pb(D[i].fi);
    sortW.pb(INF);

    ll mas = cs.sum(0, N);
    ll ans = 0;
    for(int i = 0; i <= N; i++){
        // cs.sum(0, i) -> 間違い i - cs.sum(0, i)
        // cs.sum(i, N) -> 正解
        chmax(ans, i - cs.sum(0, i) + cs.sum(i, N));

        while(sortW[i] == sortW[i+1]) i++;
    }
