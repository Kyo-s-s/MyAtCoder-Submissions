...
int main() {
    ...
    LL(N);
    VEC(ll, A, N);
    Segtree<Add_M> seg(202020);
    ll sum = 0;
    for (ll x = 1; x <= N; x++) {
        // 長さxの連続部分列 -> N - x + 1個
        // それぞれに x / 2 をかけて足し合わせる
        sum += (N - x + 1LL) * (x / 2LL);
    }    
    ...
    for (auto &g : G) {
        ...
        for (int i = 1; i < r.size(); i++) {
            seg.set(l[i - 1], seg.get(l[i - 1]) + 1);
            sum -= seg.prod(r[i] + 1, 202020) * r[i];
        }
        // reset
        for (auto e : l) seg.set(e, 0);

        for (int i = r.size() - 2; i >= 0; i--) {
            seg.set(r[i + 1], seg.get(r[i + 1]) + 1);
            sum -= seg.prod(l[i], 202020) * l[i];
        }
        for (auto e : r) seg.set(e, 0);
    }
