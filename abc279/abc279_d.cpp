...
int main() {
    ...
    auto time = [&](ll wait) -> double {
        return (double)wait * (double)B + (double)A / sqrt(1. + wait);
    };

    double Inf = 1e18; Inf += 100.;
    ...
    double ans = min(time(low), time(high));
    ll l = max(0LL, low - 10);
    ll r = high + 10;
    reps(i, l, r) {
        chmin(ans, time(i));
