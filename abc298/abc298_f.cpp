...
vll comp(vll &v) {
    map<ll, ll> mp;
    rep(i, v.size()) mp[v[i]] = 0;
    ll cnt = 0;
    for(auto &p : mp) {
        p.second = cnt++;
    }
    vll ret(v.size());
    rep(i, v.size()) ret[i] = mp[v[i]];
    return ret;
}
...
int main() {
    ...
    // R, C 座標圧縮
    vll R = comp(_R), C = comp(_C);
    ll Rm = vmax(R), Cm = vmax(C);

    vll Rsum(Rm + 1), Csum(Cm + 1);
    rep(i, N) {
        Rsum[R[i]] += X[i];
        Csum[C[i]] += X[i];
    }

    Segtree<Max_M> Cseg(Csum);
    vector<vector<pair<ll, ll>>> Rerase(Rm + 1);
    rep(i, N) {
        Rerase[R[i]].emplace_back(C[i], X[i]); 
    }

    ll ans = 0;
    for (ll r = 0; r <= Rm; r++) {
        // 更新
        for (auto [c, x] : Rerase[r]) {
            Cseg.set(c, Cseg.get(c) - x);
        }

        ll val = Rsum[r] + Cseg.all_prod();
        chmax(ans, val);
        // 戻す
        for (auto [c, x] : Rerase[r]) {
            Cseg.set(c, Cseg.get(c) + x);
