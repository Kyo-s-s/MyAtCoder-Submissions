...
int main() {
    ...
    vector dp(110, vector(110, vector(2, Mint(0))));
    dp[A][B][true] = 1;

    Mint takahashi = Mint(1) / Mint(P);
    Mint aoki = Mint(1) / Mint(Q);
    Mint ans = 0;
    ...
    for (int a = A; a < N; a++) {
        for (int b = B; b < N; b++) {
            ...
            for (int add = 1; add <= P; add++) {
                if (a + add < N) {
                    dp[a + add][b][false] += dp[a][b][true] * takahashi;
                } else {
                    ans += dp[a][b][true] * takahashi;
                }
            }
            ...
            for (int add = 1; add <= Q; add++) {
                if (b + add < N) {
                    dp[a][b + add][true] += dp[a][b][false] * aoki;
                } 
            }
        }
    }
