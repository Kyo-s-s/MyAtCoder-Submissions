...
int main() {
    ...
    auto solve_L = [&]() -> Mint {
        Mint ans = 1;
        vector<bool> used(N, false); // i の 左側 が取られているかどうか 取られていたら true
        for (auto p : P) {
            // 左側を取りたい
            // 右側にあって、聞き手が右 or 左にない 時にはWA
            if (!used[(p + 1) % N] && (S[p] == 'R' || used[p])) {
                return Mint(0);
            }
            // 右にも左にもあるとき -> 聞き手は左でなくてはならない
            // 左にしかない かつ 聞き手が ? のとき -> どちらでもよいため、ans *= 2 される
            if (S[p] == '?' && used[(p + 1) % N] && !used[p]) {
                ans *= 2;
            }
            used[p] = true;
        }
    ...
    auto solve_R = [&]() -> Mint {
        ...
        vector<bool> used(N, false); // i の 左側 が取られているかどうか 取られていたら true
        for (auto p : P) {
            if (!used[p] && (S[p] == 'L' || used[(p + 1) % N])) {
                return Mint(0);
            }
            if (S[p] == '?' && used[p] && !used[(p + 1) % N]) {
                ans *= 2;
            }
            used[(p + 1) % N] = true;
        }
