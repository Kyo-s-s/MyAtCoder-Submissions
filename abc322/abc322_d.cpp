...
struct Piece {
    ...
    Piece rotate() {
        ...
        rep(i, N) rep(j, N) {
            ret[j][N - i - 1] = data[i][j];
        }
        return Piece{ret};
    }

    optional<Piece> move(int dx, int dy) {
        vector<vector<bool>> ret(N, vector<bool>(N, false));
        rep(i, N) rep(j, N) if (data[i][j]) {
            int ni = i + dx;
            int nj = j + dy;
            if (ni < 0 || ni >= N || nj < 0 || nj >= N) return nullopt;
            ret[ni][nj] = data[i][j];
        }
        return Piece{ret};
    }
...
bool ok(Piece A, Piece B, Piece C) {
    rep(i, N) rep(j, N) {
        int c = 0;
        if (A.data[i][j]) c++;
        if (B.data[i][j]) c++;
...
int main() {
    ...
    auto getOp = [&](Piece p) -> vector<Piece> {
        vector<Piece> ret;
        rep(u, 4) {
            p = p.rotate();
            for (int dx = -5; dx <= 5; dx++) {
                for (int dy = -5; dy <= 5; dy++) {
                    auto np = p.move(dx, dy);
                    if (np) ret.pb(*np);
                }
            }
        }
