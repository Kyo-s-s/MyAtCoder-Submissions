...
int main() {
    ...
    vvb G(H, vb(W, false));
    rep(h, H) rep(w, W) {
        if (all_of(all(dxy), [&](pll d) {
            if (!include(0LL, H, h + d.fi)) return true;
            if (!include(0LL, W, w + d.se)) return true;
            return S[h + d.fi][w + d.se] == '.';
        })) {
            G[h][w] = true;
        }
    }
    ...
    auto di = [&](ll key) { return make_pair(key / W, key % W); };

    UnionFind uf(H * W);
    rep(h, H) rep(w, W) for (auto [dh, dw] : dxy) {
        ll nh = h + dh, nw = w + dw;
        if (!include(0LL, H, nh) || !include(0LL, W, nw)) continue;
        if (G[h][w] && G[nh][nw]) uf.merge(id(h, w), id(nh, nw));
    }
    ...
    for (auto &g : gr) {
        ...
        for (auto gxy : g) {
            ...
            if (G[x][y]) {
                for (auto [dx, dy] : dxy) {
                    ll nx = x + dx, ny = y + dy;
                    if (!include(0LL, H, nx) || !include(0LL, W, ny)) continue;
                    if (S[nx][ny] == '.' && !G[nx][ny]) s.insert(id(nx, ny));
                }
            }
        }

        ll add = (ll)g.size() + (ll)s.size();
        chmax(ans, add);
    }
