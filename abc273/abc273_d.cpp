...
int main() {
    ...
    while (Q--) {
        ...
        if (D == "U") {
            // WGrid[cs] の rs より小さい最大の値
            int it = upper_bound(all(WGrid[cs]), rs) - WGrid[cs].begin();
            it--;

            if (it < 0 || WGrid[cs][it] < rs - L) {
                rs -= L;
            } else {
                rs = WGrid[cs][it] + 1;
            }
        ...
        if (D == "D") {
            // WGrid[cs] の rs より大きい最小の値
            int it = lower_bound(all(WGrid[cs]), rs) - WGrid[cs].begin();

            if (it == WGrid[cs].size() || WGrid[cs][it] > rs + L) {
                rs += L;
            } else {
                rs = WGrid[cs][it] - 1;
            }
        ...
        if (D == "L") {
            // HGrid[rs] の cs より小さい最大の値
            int it = upper_bound(all(HGrid[rs]), cs) - HGrid[rs].begin();
            it--;

            if (it < 0 || HGrid[rs][it] < cs - L) {
                cs -= L;
            } else {
                cs = HGrid[rs][it] + 1;
            }
        ...
        if (D == "R") {
            // HGrid[rs] の cs より大きい最小の値
            int it = lower_bound(all(HGrid[rs]), cs) - HGrid[rs].begin();

            if (it == HGrid[rs].size() || HGrid[rs][it] > cs + L) {
                cs += L;
            } else {
                cs = HGrid[rs][it] - 1;
            }
