...
int main() {
    ...
    // p[i] := y = i となる確率
    Segtree<Add_M> segp(3 * N), p(3 * N);
    p.set(0, 1.);

    rep(i, 3 * N) {
        p.set(i, p.get(i) + segp.prod(0, i + 1));
        if (i < L) {
            ld x = p.get(i);
            segp.set(i, segp.get(i) + (x / (ld)D));
            segp.set(i + D + 1, segp.get(i + D + 1) - (x / (ld)D));
            p.set(i, 0.);
        }
    }
    ...
    rep(i, 3 * N) {
        if (i > N) q.set(i, 0.);
        else q.set(i, p.prod(0, i) + p.prod(N + 1, 3 * N));
    }
    ...
    for (int i = 3 * N - 1; i >= 0; i--) {
        ld u = r.prod(i + 1, min(3 * N, i + D + 1)) / (ld)D;
        r.set(i, max(u, q.get(i)));
    }
