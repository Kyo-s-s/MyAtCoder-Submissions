...
struct edge { 
    int from;
    ll l, d, k, c;
    edge(int from, ll l, ll d, ll k, ll c) : from(from), l(l), d(d), k(k), c(c) {}
};
...
int main() {
    ...
    while (!pq.empty()) {
        auto [di, now] = pq.top(); pq.pop();
        if (dist[now] < di) continue;
        for (auto nxt : G[now]) {
            // nxt.from -> now までの電車は、 l から幅 d で k 本あ
            // 今、 now に di 分までに乗らないとダメ
            // 一番大きい x を採用 

            if (nxt.l + nxt.c > di) continue;
            ll ok = 0, ng = nxt.k;
            while (abs(ok - ng) > 1) {
                ll mid = (ok + ng) / 2;
                if (nxt.l + nxt.d * mid + nxt.c <= di) {
                    ok = mid;
                } else {
                    ng = mid;
                }
            ...
            ll time = nxt.l + nxt.d * ok;
            if (dist[nxt.from] == -1 || dist[nxt.from] < time) {
                dist[nxt.from] = time;
                pq.push({time, nxt.from});
            }
        }
    }
