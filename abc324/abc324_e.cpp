...
int main() {
    ...
    rep(i, N) {
        int idx = 0;
        rep(j, (int)S[i].size()) {
            if (idx < T.size() && S[i][j] == T[idx]) idx++;
        }
        top[i] = idx;

        idx = T.size() - 1;
        repd(j, (int)S[i].size()) {
            if (idx >= 0 && S[i][j] == T[idx]) idx--;
        }
        tail[i] = idx;
    }


    Segtree<Add_M> seg(T.size() + 100);

    fore(x, top) x++;
    fore(x, tail) x++;

    rep(i, N) {
        seg.set(tail[i], seg.get(tail[i]) + 1);
    }

    ll ans = 0;
    rep(i, N) {
        // OUT(top[i], tail[i]);
        // ans += seg.prod(top[i] + 1, T.size() + 100);    
        // ans += seg.prod(0, top[i] + 1);
        ans += seg.prod(0, top[i]);
    }
