...
int main() {
    ...
    Mint twof = Mint(P) / Mint(100);
    Mint onef = Mint(100 - P) / Mint(100);

    vector<Mint> twov(N + 1);
    vector<Mint> onev(N + 1);
    onev[0] = 1;
    twov[0] = 1;
    for (int i = 1; i <= N; i++) {
        onev[i] = onev[i - 1] * onef;
        twov[i] = twov[i - 1] * twof;
    }
    ...
    auto f = [&](ll n) -> Mint {
        ...
        for (ll k = 0; k <= n / 2; k++) {
            ll j = n - 2 * k;
            // (P/100)^k * (1-P/100)^j * (k+j)Ck 
            Mint p = twov[k] * onev[j] * enu.nCk(k + j, k);
            // 確率p で (j + k) 回で成功する
            res += p * (j + k);
        }
        return res;
    ...
    auto g = [&](ll _n) -> Mint {
        ...
        for (ll k = 0; k <= n / 2; k++) {
            ll j = n - 2 * k;
            Mint p = twov[k + 1] * onev[j] * enu.nCk(k + j, k);
            // 確率p で (j + k) 回で成功する
            res += p * (j + k + 1);
        } 
        return res;
