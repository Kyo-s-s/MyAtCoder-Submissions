...
int main() {
    ...
    rep(i, H) {
        query.pb(make_tuple(1, i + 1, 0));
    }
    rep(i, M) {
        LL(t, a, x);
        query.pb(make_tuple(t, a, x));
    }
    reverse(all(query));

    vector<bool> row(H + 10, false), col(W + 10, false); // H, W
    ll rc = 0, cc = 0;
    vector<ll> ans(202020, 0);
    ...
    // ll su = 0;
    // rep(i, 202020) su += ans[i];
    // ans[0] += H * W - su;

    ll cnt = 0;
    rep(i, 202020) if (ans[i] > 0) cnt++;
    OUT(cnt);    
    rep(i, 202020) {
        if (ans[i] > 0) {
            OUT(i, ans[i]);
