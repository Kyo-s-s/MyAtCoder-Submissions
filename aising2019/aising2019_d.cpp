...
int main() {
    ...
    CumulativeSum<ll> cum_sum(A);
    vll B(N, 0);
    rep(i, N) if (i % 2 == 0) B[i] = A[i];
    CumulativeSum<ll> cum_diff(B);

    auto f = [&](ll x, ll p) -> bool {
        if (p >= N) return false;
        int midix = min(2 * p - 1, N);
        int cnt = midix - p + 1;
        if (!(cnt == p || cnt + 1 == p)) return false;
        return (abs(A[p] - x) <= abs(A[p - 1] - x)) && (abs(A[midix - 1] - x) <= abs(A[p - 1] - x));
    };

    auto solve = [&](ll p) -> ll {
        ll res = cum_sum.sum(0, p);
        if (2 * p < N)  {
            res += cum_diff.sum(2 * p, N);
        }
        return res;
    };
