...
int main() {
    ...
    set<pll> s;
    for (auto [x, y] : xy) {
        ll lx = uf.leader(x), ly = uf.leader(y);
        s.insert({min(lx, ly), max(lx, ly)});
    }

    // for x, y in xy, G上でxとyが非連結

    LL(Q);
    while (Q--) {
        LL(p, q); p--; q--;
        ll lp = uf.leader(p), lq = uf.leader(q);
        bool ok = s.find({min(lp, lq), max(lp, lq)}) == s.end();
        YesNo(ok);
    }
