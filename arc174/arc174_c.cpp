...
int main() {
    ...
    auto dp = [&](auto&& self, ll i) -> Mint {
        if (i <= 0) return Mint(0);
        if (vis[i]) return memo[i];

        Mint res = Mint(N - i) / Mint(N);


        res += self(self, i - 1) * (Mint(i) / Mint(N)) * (Mint((N - i + 1)) / Mint(N));

        res += self(self, i - 1) * (Mint(N - i) / Mint(N)) * (Mint(i) / Mint(N));

        res += self(self, i - 2) * (Mint(i) / Mint(N)) * (Mint(i - 1) / Mint(N));

        Mint den = Mint(1) - (Mint(N - i) / Mint(N)) * (Mint(N - i) / Mint(N));
