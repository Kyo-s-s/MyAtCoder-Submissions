...
int main() {
    ...
    vector<map<ll, ll>> mp(N);  // 高さ / 値 / 個数

    auto dfs = [&](auto&& self, int x, int y, ll val) {
        val ^= A[x][y];
        if (x + y == N - 1) {
            mp[x][val]++;
            return;
    ...
    vector<map<ll, ll>> mp2(N);  // 高さ / 値 / 個数

    auto dfs2 = [&](auto&& self, int x, int y, ll val) {
        val ^= A[x][y];
        if (x + y == N - 1) {
            mp2[x][val ^ A[x][y]]++;
            return;
