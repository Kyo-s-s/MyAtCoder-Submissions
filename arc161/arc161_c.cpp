...
template<class... Args> void FIN(const Args &...args) { OUT(args...); exit(0); }
...
void solve() {
    ...
    string T;
    rep(i, N) T.pb('!');
    auto change = [&](ll idx, char c) -> bool {
        if (T[idx] == '!') T[idx] = c;
        return T[idx] == c;
    };

    auto dfs = [&](auto &&f, ll n, ll p) -> bool {
        bool res = true;
        ll B = 0, W = 0;
        for (auto e : G[n]) if (e != p) {
            res &= f(f, e, n);
            (T[e] == 'B' ? B : W)++;
        }

        if (p == -1) {
            if (!((S[n] == 'B' && B > W) || (S[n] == 'W' && W > B))) {
                res = false;
            }         
            ll sB = 0, sW = 0;
            for (auto e : G[n]) {
