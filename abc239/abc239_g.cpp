...
int main() {
    ...
    MaxFlow<ll> graph(N * 2);
    rep(i, M) {
        LL(a, b); a--; b--;
        auto [as, at] = ch(a);
        auto [bs, bt] = ch(b);
        graph.add_edge(at, bs, INF);
        graph.add_edge(bt, as, INF);
    }
    ...
    rep(i, N) {
        auto [s, t] = ch(i);
        graph.add_edge(s, t, C[i]);
    }
 
    ll cost = graph.max_flow(1, (N - 1) * 2);
    vector<bool> mc = graph.min_cut(1);
    vector<ll> p;
    rep(i, N) {
        auto [s, t] = ch(i);
        if (mc[s] && !mc[t]) p.pb(i + 1);
    }
