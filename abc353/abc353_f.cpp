...
int main() {
    ...
    // もし、 x, y が小さいマス目上にいるなら、最寄りの(4つ) 脱出した座標(それぞれ K で割ったもの) と、そのマスへの距離
    auto dasyutu = [&](ll x, ll y) -> vector<pair<pair<ll, ll>, ll>> {
        vector<pair<pair<ll, ll>, ll>> res;
        ll dx = x / K, dy = y / K;
        if ((dx + dy) % 2 == 1) {
            res.pb(
                make_pair(make_pair(dx, dy), 0)
            );
        } else {
        // x % K, y % K について、着目すればよい
            res.pb(make_pair(make_pair(dx + 1, dy), K - x % K));
            res.pb(make_pair(make_pair(dx - 1, dy), x % K + 1));
            res.pb(make_pair(make_pair(dx, dy + 1), K - y % K));
            res.pb(make_pair(make_pair(dx, dy - 1), y % K + 1));
        }
        return res;
    };
    

    auto bigdist = [&](ll a, ll b, ll x, ll y) -> ll {
        ll dx = abs(a - x), dy = abs(b - y);
        // min(dx, dy) は斜め移動 -> *2 で行ける
        // のこりは *4 
        ll mi = min(dx, dy);
        ll ma = max(dx, dy) - mi;
        if (K == 1) {
        ...
        if (K == 2) {
            return mi * 2LL + (ma) / 2 * 3LL;
        }
        return mi * 2LL + ma * 2LL;
    };

    auto S = dasyutu(sx, sy);
    auto T = dasyutu(tx, ty);

    ll ans = INF;
    for (auto [p, u] : S) {
        for (auto [q, v] : T) {
            chmin(ans, u + v + bigdist(p.fi, p.se, q.fi, q.se));
        }
    }

    if (S.size() == 4 && T.size() == 4) {
        ll da = sx / K, db = sy / K;
        ll dx = tx / K, dy = ty / K;
        if ((da == dx) && (db == dy)) {
            ll u = abs((sx % K) - (tx % K));
