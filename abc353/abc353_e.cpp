...
struct Node {
    ...
    Node(int c, int d, Node* p) : chr(c), dep(d), parent(p) {
        children.resize(26, nullptr);
    }

    void debug_print(int depth) {
        cout << string(depth, ' ') << char('a' + chr) << " " << dep << " " << count << endl;
...
int main() {
    ...
    for (auto s : S) {
        ...
        for (auto c : s) {
            ...
            if (cur->children[idx] == nullptr) {
                cur->children[idx] = new Node(idx, cur->dep + 1, cur);
            }
    ...
    auto solve = [&](Node *node) -> void {
        ans += node->dep * (node->count - 1);
        while (node->parent != nullptr) {
            ll pc = node->parent->count;
            ll nc = node->count;
            ans += (node->parent->dep) * (pc - nc);  
            node = node->parent;
        }
    };
