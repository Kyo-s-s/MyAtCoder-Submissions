...
int main() {
    ...
    vector<vector<map<ll, pll>>> dp(N, vector<map<ll, pll>>(N));

    dp[0][0][P[0][0]] = make_pair(0, 0);

    auto update = [&](int i, int j, ll k, ll c, ll m) -> void {
        if (dp[i][j].count(k) == 0) {
            dp[i][j][k] = make_pair(c, m);
        } else {
            auto [cc, mm] = dp[i][j][k];
            if (c < cc || (c == cc && m > mm)) {
                dp[i][j][k] = make_pair(c, m);
        ...
        rep(j, N) {
            for (auto [k, v]: dp[i][j]) {
                ...
                if (i + 1 < N) {
                    ...
                    ll nk = max(k, P[i + 1][j]);
                    if (m >= cost) {
                        update(i + 1, j, nk, c + 1, m - cost);
                    } else {
                        ll mo = cost - m;
                        ll cc = (mo + k - 1) / k; 
                        ll nm = m + cc * k - cost;
                        update(i + 1, j, nk, c + cc + 1, nm);
                    }
                }
                if (j + 1 < N) {
                    ll cost = R[i][j];
                    ll nk = max(k, P[i][j + 1]);
                    if (m >= cost) {
                        update(i, j + 1, nk, c + 1, m - cost);
                    } else {
                        ll mo = cost - m;
                        ll cc = (mo + k - 1) / k; 
                        ll nm = m + cc * k - cost;
                        update(i, j + 1, nk, c + cc + 1, nm);
                    }
                }
            }
        }
