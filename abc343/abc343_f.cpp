...
struct Max_M {    
    using T = map<ll, ll, greater<ll>>; // 高々2個しか持たない！
    static T e() { return T(); }
    static T op(T x, T y) { 
        T res;
        for(auto [k, v]: x) res[k] += v;
        for(auto [k, v]: y) res[k] += v;

        // res の個数が2個になるまで、後ろを消す
        while (res.size() > 2) res.erase(prev(res.end()));
        return res;
    }
};
...
int main() {
    ...
    while (Q--) {
        ...
        if (t == 1) {
            LL(p, x); p--;
            seg.set(p, Max_M::T{{x, 1}});
        } else {
            LL(l, r); l--; r--;
            auto ans = seg.prod(l, r + 1);
            if (ans.size() == 2) {
                // OUT(ans.rbegin()->first, ans.rbegin()->second);
                OUT(ans.rbegin()->second);
            } else if (ans.size() < 2) {
                OUT(0);
            } else {
                exit(1);
