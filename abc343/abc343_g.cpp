...
// YesNo(is_include("abracadabra", "abr")); -> Yes
bool is_include(const string &S, const string &T) {
...
int main() {
    ...
    vs S;
    for (auto &s : Sp) {
        if (all_of(all(S), [&](const string &t) { return !is_include(t, s); })) {
            S.pb(s);
        }
    }
    ...
    auto solve_cost = [&](string a, string b) {
        auto Z = Zalgorithm(b + "$" + a);
        int res = 0;
        for (int i = b.size(); i < Z.size(); i++) {
            if (i + Z[i] == a.size() + b.size() + 1) {
                chmax(res, Z[i]);
            }
        }
        return res;
    ...
    for (ll msk = 0; msk < (1 << N); msk++) {
        rep(i, N) if ((msk >> i) & 1) {
            if (dp[msk][i] == -INF) continue;
            rep(j, N) if (!((msk >> j) & 1)) {
                chmax(dp[msk | (1 << j)][j], dp[msk][i] + cost[i][j]);
            }
        }
    }
