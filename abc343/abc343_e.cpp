...
int main() {
    ...
    auto check = [&](int bx, int by, int bz, int cx, int cy, int cz) -> void {
        // map<tuple<int, int, int>, int> mp;
        unordered_map<ll, int> mp;
        rep(x, 7) rep(y, 7) rep(z, 7) {
            mp[key(x, y, z)]++;
            mp[key(bx + x, by + y, bz + z)]++;
            mp[key(cx + x, cy + y, cz + z)]++;
        }
        int v1 = 0, v2 = 0, v3 = 0;
        for (auto [k, v]: mp) {
            if (v == 3) v3++;
    ...
    if (V3 == 0) {
        ...
        rep(x, 9) rep(y, 9) rep(z, 9) if (x <= y && y <= z) B.pb(make_tuple(x, y, z));
        rep(x, 9) rep(y, 9) rep(z, 9) if (x <= y && y <= z) C.pb(make_tuple(x, y, z));

        for (auto [bx, by, bz]: B) {
            for (auto [cx, cy, cz]: C) {
                check(bx, by, bz, bx + cx, by + cy, bz + cz);
            }
        }
    } else {
        vector<tuple<int, int, int>> B, C;
        reps(x, -1, 7) reps(y, -1, 7) reps(z, -1, 7) if (x <= y && y <= z) B.pb(make_tuple(x, y, z));
        rep(x, 7) rep(y, 7) rep(z, 7) C.pb(make_tuple(x, y, z));

        for (auto [bx, by, bz]: B) {
            for (auto [cx, cy, cz]: C) {
                // if (!(bx <= cx && by <= cy && bz <= cz)) continue;
                check(bx, by, bz, cx, cy, cz);
            }
        }
    }
