...
struct Trie {
    ...
    int insert(const string &s) {
        int node_id = 0;
        for (auto c : s) {
            if (nodes[node_id].next[c - 'a'] == -1) {
                nodes[node_id].next[c - 'a'] = add_node(node_id, c);
            }
            node_id = nodes[node_id].next[c - 'a'];
        }
        is_end[node_id]++;
        return node_id;
    }
    ...
    ll solve(int node_id) {
        ...
        vb used(26, false);
        while (node_id != 0) {
            int pa = parent[node_id];
            used[C[node_id] - 'a'] = true;
            rep(i, 26) if (used[i]) {
                if (nodes[pa].next[i] == -1) continue;
                res += is_end[nodes[pa].next[i]]; 
            }
            node_id = pa;
        }
        return res;
