...
int main(){
    ...
    vector dp(N+10, vector(N+10, vector(N+10, Mint(0))));
    dp[0][0][0] = 1;
    ...
    rep (i, N+1) {
        rep (j, N+1) {
            rep (k, N+1) {
                if (i + j + k == N) ans += dp[i][j][k];
                if (i + j + k > N) continue; 
                auto nxtok = [&](ll s, ll t, ll u) -> bool {
                    ll nx = A * s + C * t + E * u;
                    ll ny = B * s + D * t + F * u;
                    return NG.find({nx, ny}) == NG.end();
                };

                if (nxtok(i+1, j, k)) dp[i+1][j][k] += dp[i][j][k];
                if (nxtok(i, j+1, k)) dp[i][j+1][k] += dp[i][j][k];
                if (nxtok(i, j, k+1)) dp[i][j][k+1] += dp[i][j][k];
