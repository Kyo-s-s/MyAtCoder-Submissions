...
int main(){
    ...
    vector dp(1010, vector(1010, Mint(0)));
    dp[0][0] = 1;

    rep (i, N) {
        vector pd(1010, vector(1010, Mint(0)));
        rep (d, D + 1) {
            rep (qd, D + 1) {
                if (dp[d][qd] == 0) continue;

                for (ll x = -D; x <= D; x++) {
                    if (abs(x) + d > D) continue;
                    ll r = p[i] + x;
                    ll aqd = abs(r - q[i]);
                    if (aqd + qd > D) continue;

                    pd[d + abs(x)][qd + aqd] += dp[d][qd];
