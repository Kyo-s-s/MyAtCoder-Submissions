...
void solve() {
    ...
    rep(i, N) {
        ll a = A[i];
        if (dp[0].empty() || dp[0].back().first >= a) {
            dp[0].pb({a, i});
        } else {
            int ok = 0, ng = N;
            while (abs(ok - ng) > 1) {
                int mid = (ok + ng) / 2;
                if (dp[mid].empty() || dp[mid].back().first >= a) {
                    ng = mid;
                } else {
                    ok = mid;
                }   
    ...
    for (auto &v : dp) {
        ...
        if (now.empty()) {
            ...
            int vi = 0;
            for (auto [a, i] : v) {
                if (a >= now[vi].first) continue;
                while (vi + 1 < now.size() && a < now[vi + 1].first) {
                    vi++;
                }
                if (a < now[vi].first && i < now[vi].second) {
                    next.pb({a, i});
                }
            }
            now = next;
