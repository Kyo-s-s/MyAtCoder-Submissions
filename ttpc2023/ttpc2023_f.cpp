...
struct Parser {
    ...
    bool consume(string T) {
        string tmp = "";
        for (int i = 0; i < T.size(); i++) if (pos + i < S.size()) tmp += S[pos + i];
        if (tmp == T) {
            pos += T.size();
            return true;
        }
    ...
    State factor() {
        if (consume("N^")) {
            ...
            if (consume("^")) {
                ll num = number(); 
                ll nb = tmp.a > 0 ? 1 : 0;
                // ll nc = (tmp.b > 0 || tmp.c > 0) ? 1 : 0;
                ll nc = 1 - nb;
                return State{0, num * nb, num * nc};
            } else {
                ll nb = tmp.a > 0 ? 1 : 0;
                // ll nc = (tmp.b > 0 || tmp.c > 0) ? 1 : 0;
                ll nc = 1 - nb;
                return State{0, nb, nc};
            }
        } else if (consume("(")) {
            State tmp = expr();
            consume(")");
