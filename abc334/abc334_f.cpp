...
int main() {
    ...
    LL(Sx, Sy);
    vpll home(N);
    rep(i, N) cin >> home[i].fi >> home[i].se;

    auto sinple_dist = [&](int i, int j) -> long double {
        ll dx = home[i].fi - home[j].fi;
        ll dy = home[i].se - home[j].se; 
        return sqrt(dx * dx + dy * dy);
    };

    auto home_dist = [&](int i) -> long double {
        ll dx = home[i].fi - Sx;
        ll dy = home[i].se - Sy;
        return sqrt(dx * dx + dy * dy);
    };

    auto comp_dist = [&](int i, int j) -> long double {
        return home_dist(i) + home_dist(j);
    };


    LazySegtree<M, O, fn> seg(N);
    seg.set(0, 0); // pos 1 からスタート！S空の距離を足す

    for (int i = 1; i < N; i++) {
        // i - 1 -> i の遷移を書けばよい
        // 移動前のを取得
        ll mi = seg.prod(max(0LL, i - K), i);
        // i - 1 -> i に移動した距離を加算
        seg.apply(0, i, sinple_dist(i - 1, i));
        seg.set(i, mi + comp_dist(i - 1, i));
    ...
    long double ans = seg.prod(max(0LL, N - K), N);
    ans += home_dist(0);
    ans += home_dist(N - 1);
