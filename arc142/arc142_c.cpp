...
int main(){
    ...
    for(int i = 3; i <= N; i++){
        if(abs(one[i] - two[i]) != 1) anss = false;
    }
    ...
    if(anss){
        ...
        vi ayashi;
        for(int i = 3; i <= N; i++){
            if(min(one[i], two[i]) == 1 && max(one[i], two[i]) == 2) ayashi.pb(i);
        }
    ...
    int ans = inf;
    for(int i = 3; i <= N; i++) chmin(ans, one[i] + two[i]);
    cout << "! " << ans << endl;
}