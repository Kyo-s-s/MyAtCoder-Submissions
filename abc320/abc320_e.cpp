...
int main() {
    ...
    LL(N, M);   
    min_priority_queue<ll> hungry;
    min_priority_queue<pair<ll, ll>> eat; // time
    vll ans(N, 0);
    rep(i, N) hungry.push(i);


    rep(i, M) {
        LL(T, W, S);

        while (!eat.empty() && eat.top().fi <= T) {
            hungry.push(eat.top().se);
            eat.pop();
        }

        if (hungry.empty()) continue;
        ll e = hungry.top(); hungry.pop();
        ans[e] += W;
        eat.push({T + S, e});
    }
