...
int main() {
    ...
    vector<vector<pair<ll, pair<ll, ll>>>> G(N); // G[i] = {j, {x, y}} : i -> j に x, y
    rep(i, M) {
        cin >> A[i] >> B[i] >> X[i] >> Y[i];
        A[i]--, B[i]--;
        G[A[i]].pb({B[i], {X[i], Y[i]}});
        G[B[i]].pb({A[i], {-X[i], -Y[i]}});
    }

    vector<optional<pair<ll, ll>>> ans(N, nullopt);
    ans[0] = make_pair(0, 0);
    deque<ll> q;
    q.pb(0);
    while (!q.empty()) {
        ...
        for (auto [e, xy] : G[now]) {
            ll x = xy.fi, y = xy.se;
            if (!ans[e]) {
                ans[e] = make_pair(ans[now].value().fi + x, ans[now].value().se + y);
                q.pb(e);
            }
        }
    }
