...
int main() {
    ...
    auto solve = [&](int i) -> void {
        ...
        rep(j, N) {
            ...
            // double st = -((double)X[i] - (double)X[j]) / ((double)V[i] - (double)V[j]);
            Frac st(-(X[i] - X[j]), V[i] - V[j]);
            // double ed = -((double)X[i] - (double)X[j] + (double)A) / ((double)V[i] - (double)V[j]);
            Frac ed(-(X[i] - X[j] + A), V[i] - V[j]);

            if (st > ed) swap(st, ed);
            if (ed < Frac(0)) continue;   
            // chmax(st, Frac(0));
            if (st < Frac(0)) st = Frac(0);
            // chmax(ed, Frac(0));
            if (ed < Frac(0)) ed = Frac(0);
